...
import 'package:invitor/local_database/repository/database_client/contact_database_provider.dart';
...
class ContactsScreenState extends State<ContactsScreen>
    with ContactsBehavior, ConnectivityHandler {
  void showLoadingDialog(
      {required BuildContext context, required String taskDescription}) async {
    showDialog<void>(
        context: context,
  ...
  void createReferenceToMyContactCollection() {
    personCollection = FirebaseFirestore.instance
            ...
            TextButton(
                onPressed: () {
                  if (textUpdatingFormKey.currentState!.validate()) {
                    Navigator.pop(context);
                    contactName = convertToValidName(
                        name: textEditingController.text.trim());
                    try {
                      showLoadingDialog(
                          context: buildContext,
                          taskDescription: 'adding contact...');
                      personCollection.add({
                        'full_name': contactName,
                      });
                      textEditingController.clear();
                ...
                onPressed: () {
                  if (textUpdatingFormKey.currentState!.validate()) {
                    ...
                    try {
                      ...
                      showLoadingDialog(
                          context: buildContext,
                          taskDescription: 'updating contact...');
                      personCollection
                          .doc(firestoreContactList[index].id)
          ...
          actions: [
            ...
            TextButton(
                onPressed: () {
                  ...
                  showLoadingDialog(
                      context: buildContext,
                      taskDescription: 'Deleting contact...');
                  try {
                    personCollection
                        .doc(firestoreContactList[index].id)
                        .delete();
  ...
  Future<void> synchronizeContactsWithFirestore() async {
    showLoadingDialog(
        context: buildContext,
        taskDescription: 'restoring contact\'s backup...');
        ...
        PopupMenuItem(
          ...
          onTap: () {
            Future.delayed(const Duration(milliseconds: 0), () async {
              showLoadingDialog(
                  context: buildContext,
                  taskDescription: 'creating contact\'s backup...');
  ...
  void initState() {
    ...
    connectivitySubscription =
        connectivity.onConnectivityChanged.listen(checkMyConnectivity);
  ...
  Widget build(BuildContext context) {
    (textFieldDataMap.isNotEmpty && contactList.isNotEmpty)
        ? contactScreenTickButtonVisibility = true
        : contactScreenTickButtonVisibility = false;
                ...
                child: IconButton(
                  ...
                  onPressed: () {
                    ...
                    if (selectedContactList.isNotEmpty) {
                      ...
                      Navigator.pushNamed(
                        context,
                        CardCollectionGridViewScreen.ROUTE_NAME,
                      );
                    } else {
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                          content: Text(
                        'first_communion select a person to be invited',
                        style: TextStyle(fontFamily: 'Roboto'),
                      )));
                    }
                    // }
                                ...
                                return Material(
                                  ...
                                  /* child: Slidable(
                                    ...
                                    child: CheckboxListTile(
                                      ...
                                      onChanged: (isChecked) {
                                        setState(() {
                                          contactList[index].isChecked =
                                              isChecked!;
                                          isChecked
                                              ? inviteeCounter += 1
                                              : inviteeCounter -= 1;
                                          contactList[index].isWithFamily =
                                              false;
                                        });
                                      },
                                      secondary: Checkbox(
