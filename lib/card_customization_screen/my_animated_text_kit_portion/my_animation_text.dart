...
class _MyAnimatedTextKitState extends State<MyAnimatedTextKit>
    with TickerProviderStateMixin, ConnectivityHandler {
  ...
  Widget build(BuildContext context) {
    ...
    _currentAnimatedText.initAnimation(
        myAnimatedTextKitControllerMap[widget.animationControllerIndex]!);
    ...
    return GestureDetector(
      ...
      child: _isCurrentlyPausing ||
          ...
          ? completeText
          : AnimatedBuilder(
              animation: myAnimatedTextKitControllerMap[
                  widget.animationControllerIndex]!,
    ...
    myAnimatedTextKitControllerMap[widget.animationControllerIndex]!
      ...
      ..addListener(() async {
        if (isInAnimationToVideo) {
          framesGenerationPercentage = (((myAnimatedTextKitControllerMap[
                                  widget.animationControllerIndex]!
                              .value +
                          widget.animationControllerIndex) /
                      (myAnimatedTextKitControllerMap.length - 1)) *
                  100)
              .toInt();

          animationProgressStatusChangeNotifier!.progressStatus =
              '$framesGenerationPercentage %';

          animationToVideoCurrentStatusList[processedVideoCounter] =
              'generating frames ${animationProgressStatusChangeNotifier!.progressStatus}';

          animationProgressStatusChangeNotifier!.progressPercentageValue =
              framesGenerationPercentage;
  ...
  Future<void> _animationEndCallback(AnimationStatus state) async {
    if (isInAnimationToVideo) {
      if (state == AnimationStatus.forward) {
        for (int i = processedVideoCounter;
            i < animationToVideoProcessControlList.length;
            i++) {
          if (i == processedVideoCounter) {
            animationToVideoProcessControlList[i] = true;
            animationToVideoCurrentStatusList[i] =
                'generating frames ${animationProgressStatusChangeNotifier!.progressStatus}';
            animationProgressStatusChangeNotifier!.status =
                'generating frames ${animationProgressStatusChangeNotifier!.progressStatus}';
          } else {
    ...
    if (state == AnimationStatus.completed) {
      ...
      if (widget.animationControllerIndex <
              (myAnimatedTextKitControllerMap.length - 2) &&
          isPreviewEnable) {
        widget.animationControllerIndex += 1;
        animatedTextContainerVisibilityList[widget.animationControllerIndex] =
            true;
        myAnimatedTextKitControllerMap[widget.animationControllerIndex]!
            .reset();
        cardCustomizationScreenChangeNotifier
        ...
        if (isInAnimationToVideo) {
          ...
          //image invitation
          currentImageTitle =
              '${mainCategoryTitleMap[mainCategoryTitle]}${subCategoryTitleMap[mainCategoryTitle]![subCategoryTitle]}-${selectedContactList[processedVideoCounter].name.replaceAll(' ', '')}_${DateTime.now().toString().replaceAll(' ', '').replaceAll(':', '').replaceAll('-', '').replaceAll('.', '')}.png';
          currentlyProcessingFilePath =
              'storage/emulated/0/Invitor/Images/$currentImageTitle';

          animationToVideoCurrentStatusList[processedVideoCounter] =
              'preparing invitation (image)...';
          animationProgressStatusChangeNotifier!.status =
              'preparing invitation (image)...';

          Uint8List imageInvitation = await File(
                  '$temporaryFramesDirectory/frame_${namedCounterOfCard - 2}.png')
              .readAsBytes();
          ...
          //video invitation
          animationToVideoCurrentStatusList[processedVideoCounter] =
              'preparing invitation (video)...';
          animationProgressStatusChangeNotifier!.status =
              'preparing invitation (video)...';

          currentVideoTitle =
              '${mainCategoryTitleMap[mainCategoryTitle]}${subCategoryTitleMap[mainCategoryTitle]![subCategoryTitle]}-${selectedContactList[processedVideoCounter].name.replaceAll(' ', '')}_${DateTime.now().toString().replaceAll(' ', '').replaceAll(':', '').replaceAll('-', '').replaceAll('.', '')}.mp4';
          currentlyProcessingFilePath =
              'storage/emulated/0/Invitor/Videos/$currentVideoTitle';

          await flutterFFmpeg.execute(
              '-r 25 -f image2 -i $temporaryFramesDirectory/frame_%d.png -vcodec libx264 -crf 25  -pix_fmt yuv420p storage/emulated/0/Invitor/Videos/temp_$currentVideoTitle');

          Directory appDocDir = await getApplicationDocumentsDirectory();
          await flutterFFmpeg.execute(
              '-i storage/emulated/0/Invitor/Videos/temp_$currentVideoTitle -i ${appDocDir.path}/watermark.png -filter_complex "overlay=main_w-overlay_w-5:main_h-overlay_h-5:format=auto,format=yuv420p" -c:a copy storage/emulated/0/Invitor/Videos/$currentVideoTitle');

          //gif invitation
          if (animationProgressStatusChangeNotifier!.gifInvitation) {
            currentGifTitle =
                '${mainCategoryTitleMap[mainCategoryTitle]}${subCategoryTitleMap[mainCategoryTitle]![subCategoryTitle]}-${selectedContactList[processedVideoCounter].name.replaceAll(' ', '')}_${DateTime.now().toString().replaceAll(' ', '').replaceAll(':', '').replaceAll('-', '').replaceAll('.', '')}.gif';
            currentlyProcessingFilePath =
                'storage/emulated/0/Invitor/Gifs/$currentGifTitle';

            animationToVideoCurrentStatusList[processedVideoCounter] =
                'preparing invitation (gif)...';
            animationProgressStatusChangeNotifier!.status =
                'preparing invitation (gif)...';

            await flutterFFmpeg
                .execute(
                    '-i storage/emulated/0/Invitor/Videos/$currentVideoTitle -vf "fps=24,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse" -loop 0 storage/emulated/0/Invitor/Gifs/$currentGifTitle')
          ...
          try {
            await File(
                    'storage/emulated/0/Invitor/Videos/temp_$currentVideoTitle')
                .delete();
          ...
          tickMarkList[processedVideoCounter] = const Icon(
            Icons.done,
          ...
          Future.delayed(const Duration(seconds: 1), () async {
            ...
            if (processedVideoCounter < selectedContactList.length) {
              ...
              formalInvitationHeaderText =
                  '${selectedContactList[processedVideoCounter].name}$formalInvitationHeaderTextPartOne${selectedContactList[processedVideoCounter].isWithFamily ? withYourFamilyTextFormat : ''}$formalInvitationHeaderTextPartTwo';
              ...
              animationProgressStatusChangeNotifier!.startTextButtonVisibility =
                  true;
              for (int i = 0;
